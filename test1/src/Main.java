...
class Process {
    ...
    public int  getWaitingTime(int cTime) { 
    	int waitTime=cTime-arrivalTime;
    	return waitTime; 
    } 

    // 문제 2: 앞으로 더 실행해야 하는 남은 실행시간을 반환함
    public int  getRemainingTime()        { 
    	int remainTime=serviceTime-executionTime;
    	return remainTime; 
    } 
...
{ 
	...
	public void schedule() {
		...
		if(!readyQueue.isEmpty()) {
			Process shortProcess = readyQueue.get(0);
			for(Process p: readyQueue) {
				if(p.getServiceTime() < shortProcess.getServiceTime()) {
					shortProcess = p;
				}
			}
			currentProcess = shortProcess;
		}
	}
}
...
class HRRN extends Scheduler
{
	...
	public void schedule() {
		...
		if(!readyQueue.isEmpty()) {
			...
			for(Process p: readyQueue) {
				double responseRatio = p.getResponeRatioTime(currentTime);
				if(responseRatio > maxProcess) {
					maxProcess = responseRatio;
					process = p;
				}
			}
...
class SRT extends Scheduler
{
	...
	public void schedule() {
		super.schedule();
		if(!readyQueue.isEmpty()) {
			Process minProcess = readyQueue.get(0);
			for(Process p: readyQueue) {
				if(p.getRemainingTime()<minProcess.getRemainingTime()) {
					minProcess = p;
				}
			}
...
class RR extends Scheduler
{
	...
	public void schedule() {
		super.schedule();
		if(timeQuantumExausted()&&!readyQueue.isEmpty()) {
