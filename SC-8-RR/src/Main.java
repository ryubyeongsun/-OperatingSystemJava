...
class Process {
    ...
    public void incExecTime()          {  executionTime+=1;} 
    ...
    public double  getResponeRatioTime(int cTime)  { 
        return (double)getWaitingTime(cTime)/serviceTime+1;
    }
...
class SPN extends Scheduler {
    ...
    public void schedule() {
        ...
        Process smallProcess = null;
        for (Process p : readyQueue) {
            if (smallProcess==null|| p.getServiceTime() < smallProcess.getServiceTime()) {
                smallProcess = p;
            }
        }

        currentProcess = smallProcess; 
    }
...
class HRRN extends Scheduler {
    ...
    public void schedule() {
        ...
        Process bigHRRNProcess = null;

        for (Process p : readyQueue) {
           

            if (bigHRRNProcess == null || p.getResponeRatioTime(currentTime)> bigHRRNProcess.getResponeRatioTime(currentTime)) {
                bigHRRNProcess = p;
            
            }
        }

        currentProcess = bigHRRNProcess;
    }
}
...
class SRT extends Scheduler {
    ...
    public void schedule() {
        ...
        for (Process p : readyQueue) {
            if (p.getRemainingTime() < smallTime) {
                smallTime = p.getRemainingTime();
                currentProcess = p;
            }
        }
    }
  ...
  @Override
    public boolean isSchedulable() {
        return super.isSchedulable() || !readyQueue.isEmpty();
    }
}
...
class ComputerSystem 
{
    ...
    public void printEpilog(Scheduler scheduler) {
    	System.out.print("Scheduling Algorithm: " + scheduler.getName() + "\n");
        for (int i = 0; i <= 50; ++i) {
            if (i % 10 == 0) {
